...
class Diamond:
    ...
    def __init__(self, x: int, y: int, width: int, height: int, z: int):
        ...
        self.coords_x = [x,  x + width,  x - height,  x + width - height]
        self.coords_y = [y,  y + width,  y + height,  y + width + height]
        self.coeffs = [1, -1, -1, 1]
...
class Feature2hRot(FeatureRot):
    def __init__(self, x: int, y: int, dx: int, dy: int, z: int):
        ...
        self.coords_x = [x,  x + dx*2,  x - dy,  x + dx*2 - dy,
                         x,  x + dx,  x - dy,  x + dx - dy]

        self.coords_y = [y,  y + dx*2,  y + dy,  y + dx*2 + dy,
                         y,  y + dx,  y + dy,  y + dx + dy]
...
class Feature2vRot(FeatureRot):
    def __init__(self, x: int, y: int, dx: int, dy: int, z: int):
        ...
        self.coords_x = [x, x + dx, x - 2*dy, x + dx - 2*dy,
                         x, x + dx, x - dy,   x + dx - dy]

        self.coords_y = [y, y + dx, y + 2*dy, y + dx + 2*dy,
                         y, y + dx, y + dy,   y + dx + dy]
...
class Feature3hRot(FeatureRot):
    def __init__(self, x: int, y: int, dx: int, dy: int, z: int):
        ...
        self.coords_x = [x,       x + dx*3,  x - dy,  x + dx*3 - dy,
                         x + dx,  x + 2*dx,  x + dx - dy,  x + 2*dx - dy]

        self.coords_y = [y,       y + dx*3,     y + dy,      y + dx*3 + dy,
...
class Feature3vRot(FeatureRot):
    def __init__(self, x: int, y: int, dx: int, dy: int, z: int):
        ...
        self.coords_x = [x,      x + dx,      x - 3*dy,   x + dx - 3*dy,
                         x - dy, x - dy + dx, x - 2*dy,   x + dx - 2*dy]

        self.coords_y = [y,      y + dx,      y + 3*dy,   y + dx + 3*dy,
...
class Feature4hRot(FeatureRot):
    def __init__(self, x: int, y: int, dx: int, dy: int, z: int):
        ...
        self.coords_x = [x,       x + dx*4,  x - dy,  x + dx*4 - dy,
                         x + dx,  x + 3*dx,  x + dx - dy,  x + 3*dx - dy]

        self.coords_y = [y,       y + dx*4,     y + dy,      y + dx*4 + dy,
...
class Feature4vRot(FeatureRot):
    def __init__(self, x: int, y: int, dx: int, dy: int, z: int):
        ...
        self.coords_x = [x,      x + dx,      x - 4*dy,   x + dx - 4*dy,
                         x - dy, x - dy + dx, x - 3*dy,   x + dx - 3*dy]

        self.coords_y = [y,      y + dx,      y + 4*dy,   y + dx + 4*dy,
...
class Feature2h2vRot(FeatureRot):
    def __init__(self, x: int, y: int, dx: int, dy: int, z: int):
        ...
        self.coords_x = [x,              x + 2*dx,             x - 2*dy,   x + 2*dx - 2*dy,
                         x,                x + dx,               x - dy,       x + dx - dy,
                         x + dx - dy,    x + 2*dx - dy,   x + dx - 2*dy,   x + 2*dx - 2*dy]

        self.coords_y = [y,              y + 2*dx,        y + 2*dy,   y + 2*dx + 2*dy,
                         y,                y + dx,          y + dy,       y + dx + dy,
...
class Feature3h3vRot(FeatureRot):
    def __init__(self, x: int, y: int, dx: int, dy: int, z: int):
        ...
        self.coords_x = [x,       x + 3*dx,       x - 3*dy,  x + 3*dx - 3*dy,
                         x,         x + dx,       x - dy,        x + dx - dy]

        self.coords_y = [y,          y + 3*dx,  y + 3*dy,  y + 3*dx + 3*dy,
                         y + dy,  y + dy + dx,  y + 2*dy,  y + dx + 2*dy]
