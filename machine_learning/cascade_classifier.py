...
class CascadeClassifier2:
    ...
    def predict(self, candidate_features: pd.DataFrame, features_set:str = 'all_features'):
        ...
        model1_probas = self.first_model.predict_proba(candidate_features[features_sets[features_set]])[:, 1]
        model2_probas = self.second_model.predict_proba(candidate_features[features_sets[features_set]])[:, 1]
        model2_probas[model1_probas <= self.max_conf_thr_required] = 0
        return model2_probas
